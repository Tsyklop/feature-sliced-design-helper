...
public class NewLibSharedDialog extends BaseSharedDialog<DefaultSharedForm> {
    ...
    public NewLibSharedDialog(@NotNull Project project, @NotNull JsLibraryType jsLibrary) {
        super("New Shared Lib", new DefaultSharedForm(), project, jsLibrary);
        init();
        initValidation();
    }
        ...
        CommandProcessor.getInstance().executeCommand(project, () -> {
            ...
            try {
                ...
                VirtualFile createdComponentFile = ApplicationManager.getApplication().runWriteAction((ThrowableComputable<VirtualFile, IOException>) () -> {
                    ...
                    final VirtualFile sharedUiDir = VfsUtil.createDirectoryIfMissing(projectRoot, buildPathToSegment(SegmentType.LIB));
